...
| [`02_demo`](02_demo) | Application complète : lecture, recherche, édition en staging, upload de photos, interface FR/EN et affichage de la réponse JSON brute. En ligne : <https://openfoodfacts-php-demo.fly.dev/> |
...
`02_demo` est une application autonome, avec son propre `composer.json` :

```sh
cd examples/02_demo && composer install && composer start
```
